...
function Install()
{
  ...
  URLKernel='https://github.com/hectorsiglo21/TEST/lotServer-1/raw/main/lotServer.log'
  ...
  mkdir -p "${AcceTmp}/bin/"
  mkdir -p "${AcceTmp}/etc/"
  wget --no-check-certificate -qO "${AcceTmp}/bin/${AcceBin}" "https://github.com/hectorsiglo21/TEST/lotServer-1/raw/main/${MyKernel}"
  [ ! -f "${AcceTmp}/bin/${AcceBin}" ] && Uninstall "Download Error! Not Found ${AcceBin}. "
  Welcome;
  wget --no-check-certificate -qO "/tmp/lotServer.tar" "https://github.com/hectorsiglo21/TEST/lotServer-1/raw/main/lotServer.tar"
